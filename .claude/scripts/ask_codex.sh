...
#   AI_EXCHANGE_SLUG=periodic_thin_cut ask_codex.sh "..."   # pin the per-task topic file
...
# vendor config drifts, the defaults here stay put and the two simply diverge. Leaving either unset still works,
# but the entry is stamped [defaulted] and a warning goes to stderr — an unchosen tier must be
# visible in the record, because "the jury agreed" means little without knowing how hard it looked.
#
...
FULL_PROMPT="You are the Codex AI conducting a read-only code audit for the BELFEM project.
Before answering, read these two files in the project root:
  - AGENTS.md
  - doc/ai_collaboration_protocol.md
They define your role (precision audit), the audit checklist, and the required format.
...
- State a confidence tier (high / medium / low) and ground it in evidence.
