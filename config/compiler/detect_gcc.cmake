...
# step 1: resolve any symlink (use CMake instead of shell tools for BSD/GNU portability)
set( _BELFEM_CXX_COMPILER "${CMAKE_CXX_COMPILER}" )
if( NOT _BELFEM_CXX_COMPILER AND DEFINED ENV{CXX} AND NOT "$ENV{CXX}" STREQUAL "" )
    set( _BELFEM_CXX_COMPILER "$ENV{CXX}" )
endif()

if( NOT IS_ABSOLUTE "${_BELFEM_CXX_COMPILER}" )
    find_program( _BELFEM_CXX_COMPILER_PATH NAMES "${_BELFEM_CXX_COMPILER}" )
else()
    set( _BELFEM_CXX_COMPILER_PATH "${_BELFEM_CXX_COMPILER}" )
endif()

if( NOT _BELFEM_CXX_COMPILER_PATH )
    message( FATAL_ERROR "Could not resolve C++ compiler path: ${_BELFEM_CXX_COMPILER}" )
endif()

get_filename_component( BELFEM_TPATH "${_BELFEM_CXX_COMPILER_PATH}" REALPATH )
