...
## Code findings, tier 5 (2026-09-18)

F22, F23 and F24 in one loop, astra on the Codex seat. F22 was the order-of-operations defect tier 4 had filed: `Calculator::set_integration_order()` let `allocate_memory()` precompute the sideset's edge functions from the group's tables and only then asked the group to rebuild them at the new order, so the edge functions kept the old point count. Latent, because every caller passed the order the tables already held. The fix moves the guarded rebuild ahead of the allocation. The gate is a shipped regression test on the ghost-stack fixture (`tests/fem/test_SideSetRebuild.cpp`): read the master table's point count through the group, set order 13, link the ghost element, evaluate the master and slave edge functions at the last point. On the unfixed tree it fails with the bounds assertion, seven points precomputed against a rule of thirty-seven; after the fix it passes. Debug-only by the header's own rule, since the assertion is what catches it. F23: `~Block` never freed its enrichment tables; a `delete_lookup_tables()` helper mirrors tier 4 and runs first in the initializer and from the destructor. Codex pointed out that a parentless block enriches TRI and TET geometry unconditionally, so the gate is a second test (`tests/fem/test_BlockRebuild.cpp`, compiled without Maxwell) that builds such a block twice and deletes it, run under Valgrind: one record at the block's initializer before the fix, none after. F24: the dead sideset member deleted.

The plan jury approved all three and corrected the test: my first version read the calculator's cached table pointer, which is null before the first element link and dangling after a rebuild, and it segfaulted on its first run exactly as both auditors predicted. The code jury approved the production edits and corrected the contracts once more: a facet link refreshes the cached table pointers but not the edge-function pointers, and those pointers dangle rather than go stale, since `allocate_memory()` deletes the objects behind them. Also from the jury: a trailing newline, the Maxwell gate the block test did not need, a slave-side check in the sideset test. Codex found one more leak on the same path, filed as F25: `allocate_memory()` clears the calculator's vector list without deleting the objects, so every setter call after the first loses a generation; not seen in the deck gates, and the IWGs borrow those vectors, so the fix needs a borrower audit first.

Gates: `make check` on both trees, 17 of 17, the debug suite rerun after the test split; Valgrind on the block test before and after the fix as above; fresh Valgrind on both decks: no record cites the sideset file or the block's initializer on any rank, no invalid access, the dipole's result file byte-identical to the tier-1 run, the 3D deck's tape current identical to 15 digits and its rank losses unchanged from tier 4 (the swap changes no table on today's decks, so this is non-regression, not F22 evidence: the test is the F22 gate). The peer's uncommitted `cl_CutFactory` edits were in the working tree during the builds and are not part of this commit.
