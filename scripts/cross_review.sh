...
# under flock and raises tmp/ai_exchange/AUTOREVIEW_P0.flag on P0/CRITICAL findings.
...
# pin the toolchain before anything runs — hook invocations have a bare environment
. "$ROOT/scripts/scls_env.sh"
...
SUBJECT_FILE=$(mktemp "${TMPDIR:-/tmp}/crossrev_subject_XXXXXX.md")
PROMPT_FILE=$(mktemp "${TMPDIR:-/tmp}/crossrev_prompt_XXXXXX.md")
...
PRIME_JURY="You are an independent reviewer. Cite file:line for every claim. State a confidence level (high / medium / low) per finding."
...
build_prompt() {  # $1 = priming
    { printf '%s\n\n' "$1"
      ...
      printf 'Review the content below for defects, risks, and violations of doc/coding_philosophy.md. Read-only — modify nothing.\n\n'
