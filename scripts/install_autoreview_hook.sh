...
# The hook is a no-op unless BELFEM_AUTOREVIEW=1 and never blocks the commit.
...
cat > "$HOOK" <<'EOF'
#!/usr/bin/env bash
# belfem-autoreview — opt-in background light review. Must NEVER block the commit
# and never touch the tree. Runs in whatever worktree the commit happened in.
[ "${BELFEM_AUTOREVIEW:-0}" = "1" ] || exit 0
...
# pin the toolchain (idempotent); a failed pin skips the review (loudly), never the commit
. "$TOP/scripts/scls_env.sh" || exit 0
nohup nice -n 19 "$TOP/scripts/cross_review.sh" --quick "$SHA" >/dev/null 2>&1 &
exit 0
EOF
chmod +x "$HOOK"
echo "installed $HOOK"
echo "opt-in per shell: export BELFEM_AUTOREVIEW=1 (default OFF; covers all worktrees)"
