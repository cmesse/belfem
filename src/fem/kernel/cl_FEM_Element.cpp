...
//------------------------------------------------------------------------------
        ...
        Element::Element(
                ...
                mElement( aElement )
        {
            // aura elements carry no dofs, but the postprocessors evaluate
            // their nedelec fields, which need the edge orientations
            if ( mElement->has_edges() )
            {
...
//------------------------------------------------------------------------------
        ...
        {
            id_t tMasterBlockID = aMaster == nullptr ? 0 : aMaster->element()->block_id() ;
            id_t tSlaveBlockID = aSlave == nullptr ? 0 : aSlave->element()->block_id() ;

            if( mMaster == nullptr )
            {
                mMaster = mSlave ;
                mSlave = nullptr ;
                tMasterBlockID = tSlaveBlockID ;
                tSlaveBlockID = 0 ;
            }
...
//------------------------------------------------------------------------------
                ...
                const id_t                  aSlaveBlockID  )
        {
            ...
            DomainType tType = mParent->parent()->mesh()->sideset( aSideSetID )->domain_type() ;

            if ( tType == DomainType::ThinShell || tType == DomainType::Inactive  || tType == DomainType::GeometryOnly )
            {
            ...
            switch ( tMode )
            {
                ...
                case( SideSetDofLinkMode::Inactive ) :
                {
                    // pass, either this sideset is not linked or
                    // we have to do this directly over the factory
                    // (e.g for thin shells)
                    break ;
                }
                default :
...
#ifdef DEBUG
                // Only compare when master and slave share the same element
                // type (ghost pattern). For heterogeneous pairings the bitsets
                // are not index-comparable.
                if ( aSlaveEdgeDofTypes.length() > 0
                     && mMaster->element()->type() == mSlave->element()->type() )
                {
                    Bitset< 12 > tEdgeDirections ;
                    mSlave->edge_directions( tEdgeDirections );
                    BELFEM_ERROR( tEdgeDirections == mEdgeDirections , "Edge directions do not match");
                }
#endif
...
//------------------------------------------------------------------------------
        ...
        Element::compute_edge_directions()
        {
            ...
            for( uint e=0; e<tNumEdges; ++e )
            {
                ...
                if ( tA == tNodes(0)->original()->id() && tB == tNodes(1)->original()->id() )
                {
                    mEdgeDirections.set( e );
                }
                else if ( tA == tNodes(1)->original()->id() && tB == tNodes(0)->original()->id() )
                {
                    mEdgeDirections.reset( e );
                }
                else
                {
                    ...
                    Vector< real > tCx( 3 );
                    Vector< real > tDx( 3 );
                    mElement->edge( e )->node( 0 )->original()->get_coords( tAx );
                    mElement->edge( e )->node( 1 )->original()->get_coords( tBx );
                    tNodes(0)->original()->get_coords( tCx );
                    tNodes(1)->original()->get_coords( tDx );

                    real tP = norm( tAx-tCx );
                    real tQ = norm( tBx-tDx );
                    real tR = norm( tAx - tDx );
                    real tS = norm( tBx - tCx );

                    if ( tP < BELFEM_MESH_EPSILON && tQ < BELFEM_MESH_EPSILON )
                    {
                        mEdgeDirections.set( e );
                    }
                    else if ( tR < BELFEM_MESH_EPSILON && tS < BELFEM_MESH_EPSILON )
                    {
                        mEdgeDirections.reset( e );
                    }
                    else
                    {
                        ...
                        tCx.print( "C" );
                        tDx.print( "D" );
                        BELFEM_ERROR( false, "Error when trying to compute edge direction of element %lu of type %s (edges: %lu->%lu vs. %lu->%lu",
                                     ( long unsigned int ) this->id(),
                                     to_string( mElement->type() ).c_str(),
                                     ( long unsigned int ) tA,
                                     ( long unsigned int ) tB,
                                     ( long unsigned int ) tNodes(0)->original()->id(),
                                     ( long unsigned int ) tNodes(1)->original()->id() );
                    }
                }
...
//------------------------------------------------------------------------------
        ...
        Element::compute_edge_directions_thinshell()
        {
            BELFEM_ASSERT(
                    ...
                    "the ThinShellFactory does not compute edge directions for thin shells in 3d yet!");

            // this is actually done by the ThinShellFactory.
...
//------------------------------------------------------------------------------
        ...
        void
        Element::relink_dofs( Cell< Dof * > & aGlobalDofs,
                              Cell< Dof * > & aLocalDofs,
                              const Matrix< real > & aTmatrix  )
        {
            ...
            uint tCount = 0 ;
            for( Dof * tDof : aGlobalDofs )
            {
                mDOFs[ tCount++ ] = tDof ;
            }

            tCount = 0 ;
            mNumberOfLocalDofs = aLocalDofs.size() ;
            mLocalDofs = ( Dof ** ) malloc( mNumberOfLocalDofs * sizeof( Dof *  ) );
            for( Dof * tDof : aLocalDofs )
            {
            ...
            BELFEM_ASSERT( mNumberOfLocalDofs == aTmatrix.n_rows() && mNumberOfDofs == aTmatrix.n_cols(),
                           ...
                           ( long unsigned int ) this->id(),
                           ( unsigned int ) aTmatrix.n_rows(),
                           ( unsigned int ) aTmatrix.n_cols(),
                           ( unsigned int ) mNumberOfLocalDofs,
                           ( unsigned int ) mNumberOfDofs );
...
//------------------------------------------------------------------------------
        ...
        bool
        Element::has_hanging_dofs() const
        {

            for( uint k=0; k<mNumberOfDofs; ++k )
            {
                if( mDOFs[ k ]->is_hanging() )
                {
                    return true ;
                }
            }
