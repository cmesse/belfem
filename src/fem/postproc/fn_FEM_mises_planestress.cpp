        ...
        mises_planestress( DofManagerBase * aField )
        {
            if( comm_rank() == 0 )
            {
                ...
                for ( id_t tBlockID : aField->iwg()->selected_blocks() )
                {
                    ...
                    BELFEM_ASSERT( tBlock->material() != nullptr, "no material was assigned to block %u",
                                  ( unsigned int ) tBlock->id() );

                    BELFEM_ERROR( false, "missing implementation: the C-matrix is not populated");
                    ...
                    for( mesh::Node * tNode : tMesh->nodes() )
                    {
                        ...
                        if( tNode->is_flagged() )
                        {
                            ...
                            // populate elasticity
                            //tMat->C_ps( tC ); //#HACK, C_ps doesn't exist after refactoring...
                    ...
                    message( InfoLevel::Verbose, "    ... time for computing Stresses             : %u ms\n",
