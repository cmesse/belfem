...
//------------------------------------------------------------------------------
               ...
               Vector< real > & aValues )
    {
        ...
        // arma::eig_sym writes into an arma::Col, but Vector< T > is backed by an
        // arma::Mat, so the eigenvalues are collected in a local and copied out
        arma::Col< real > tValues ;

        BELFEM_ERROR( arma::eig_sym( tValues, aMatrix.matrix_data() ),
                      "eigen_sym(): the symmetric eigenvalue decomposition did not converge" );
