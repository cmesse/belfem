...
{
    /**
     * @brief Dense column-major matrix. The backend (Armadillo or Blaze) is chosen at
     * build time; both provide the same belfem::Matrix interface.
     *
     * @ingroup grp_linalg
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        {
            const VT & tVector = static_cast< const VT& >( aExpression );
            mMatrix.resize( tVector.size(), 1UL, false );

            for ( size_t k = 0; k < tVector.size(); ++k )
            {
                mMatrix( k, 0 ) = tVector[ k ];
            }
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        {
            const VT & tVector = static_cast< const VT& >( aExpression );
            mMatrix.resize( 1UL, tVector.size(), false );

            for ( size_t k = 0; k < tVector.size(); ++k )
            {
                mMatrix( 0, k ) = tVector[ k ];
            }
        }
...
//------------------------------------------------------------------------------
        ...
        */
        ...
        Matrix( Matrix< T > && aMatrix ) noexcept :
                mMatrix( std::move( aMatrix.mMatrix ) )
        {}
...
//------------------------------------------------------------------------------
        ...
        /**
         * inter-column stride of the data container
         * ( the leading dimension in BLAS terms; exceeds n_rows when
         *   blaze pads columns for SIMD alignment )
         */
        inline size_t
        spacing() const
        {
            return mMatrix.spacing();
        }
...
//------------------------------------------------------------------------------
         ...
         */
        Matrix< T > &
        operator=( Matrix< T > && aMatrix ) noexcept
        {
            if ( this != & aMatrix )
            {
                mMatrix = std::move( aMatrix.mMatrix );
...
//------------------------------------------------------------------------------
        ...
        inline void
        set_row( const size_t aRowIndex, const ET & aExpression )
        {
            this->row( aRowIndex ) = aExpression;
        }

//------------------------------------------------------------------------------

        inline void
        set_col( const size_t aColIndex, const Vector <T> & aVector )
        {
            // make sure that length is correct
            ...
            std::copy(  aVector.data(), aVector.data() + aVector.length(),
                    this->col( aColIndex ).data() );
        }

        template < typename ET >
        inline void
        set_col( const size_t aRowIndex, const ET & aExpression )
        {
            this->col( aRowIndex ) = aExpression;
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        template < typename MT, bool SO >
        Matrix< T > &
        operator=( const blaze::Matrix< MT, SO > & aExpression )
        {
            mMatrix = aExpression;
            return *this;
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        template < typename VT >
        Matrix< T > &
        operator=( const blaze::DenseVector< VT, false > & aExpression )
        {
            const VT & tVector = static_cast< const VT& >( aExpression );
            mMatrix.resize( tVector.size(), 1UL, false );
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        template < typename VT >
        Matrix< T > &
        operator=( const blaze::DenseVector< VT, true > & aExpression )
        {
            const VT & tVector = static_cast< const VT& >( aExpression );
            mMatrix.resize( 1UL, tVector.size(), false );
