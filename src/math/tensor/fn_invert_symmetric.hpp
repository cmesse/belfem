...
//----------------------------------------------------------------------------
    ...
    namespace tensor
    {
                ...
                Vector< int_t  > & aPivot )
        {
            ...
            lapack::getrf( &n, &n, tWorkA, &n, aPivot.data(), &info );

            BELFEM_ERROR( info == 0, "LAPACK getrf has thrown error %i", ( int ) info );

            // create the inverse of A using the LU factorization
            lapack::getri( &n, tWorkA, &n, aPivot.data(), tWorkB, &lwork, &info );

            BELFEM_ERROR( info == 0, "LAPACK getri has thrown error %i", ( int ) info );
