...
//------------------------------------------------------------------------------
            ...
            Matrix< real > & aMatrix )
    {
        ...
        tData[ 8 ] = cb*cc;
#elif  BELFEM_BLAZE
        // Use the (i,j) accessor: Blaze pads columns for SIMD alignment, so
        // the inter-column stride is not necessarily 3 and manual offsets
        // into aMatrix.data() are unreliable.
        aMatrix( 0, 0 ) = ca*cb ;
        aMatrix( 1, 0 ) = cb*sa ;
        aMatrix( 2, 0 ) = -sb ;

        aMatrix( 0, 1 ) = ca*sb*sc - cc*sa ;
        aMatrix( 1, 1 ) = ca*cc + sa*sb*sc ;
        aMatrix( 2, 1 ) = cb*sc ;

        aMatrix( 0, 2 ) = sa*sc + ca*cc*sb ;
        aMatrix( 1, 2 ) = cc*sa*sb - ca*sc ;
        aMatrix( 2, 2 ) = cb*cc;
#endif
