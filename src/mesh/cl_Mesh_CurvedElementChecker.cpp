...
//------------------------------------------------------------------------------
        ...
        CurvedElementChecker::check_quad4_face( Element * aElement, const uint aFaceIndex )
        {
            ...
            // check if point does not sit on plane
            return this->check_quad4_face_sub() ;
        }

        bool
        CurvedElementChecker::check_quad4_face_sub()
        {
            mX[ 0 ] = mNodes( 0 )->x();
            mX[ 1 ] = mNodes( 1 )->x();
            mX[ 2 ] = mNodes( 2 )->x();
...
//------------------------------------------------------------------------------
        ...
        {
            // note: we need to do this manually since penta6 could be a TS element

            mNodes.set_size( 4, nullptr );

            // face 0
            mNodes( 0 ) = aElement->node( 0 );
            mNodes( 1 ) = aElement->node( 1 );
            mNodes( 2 ) = aElement->node( 4 );
            mNodes( 3 ) = aElement->node( 3 );

            if ( this->check_quad4_face_sub() )
            {
                return true ;
            }

            // face 1
            mNodes( 0 ) = aElement->node( 1 );
            mNodes( 1 ) = aElement->node( 2 );
            mNodes( 2 ) = aElement->node( 5 );
            mNodes( 3 ) = aElement->node( 4 );
            ...
            // face 2
            mNodes( 0 ) = aElement->node( 2 );
            mNodes( 1 ) = aElement->node( 0 );
            mNodes( 2 ) = aElement->node( 3 );
            mNodes( 3 ) = aElement->node( 5 );
