...
//------------------------------------------------------------------------------
        ...
        void
        ExodusWriter::collect_blocks()
        {
#ifdef BELFEM_EXODUS
            struct
            {
                inline bool
                operator()( const Block * aA, const Block * aB )
                {
                    return aA->id() < aB->id();
                }
            } opBlockId ;
            ...
            for ( Block * tBlock : mMesh->blocks() )
            {
                if ( tBlock->number_of_elements() > 0 )
                {
                    mBlocks( tCount++ ) = tBlock ;
                }
            }

            sort( mBlocks, opBlockId );
#endif
        }
...
//------------------------------------------------------------------------------
        ...
        ExodusWriter::populate_header()
        {
            ...
            // exodus requires num_elem == sum over the defined blocks
            mNumElements = 0 ;
            ...
            for( SideSet * tSideSet : mMesh->sidesets() )
            {
                if( ! tSideSet->is_hidden() && tSideSet->number_of_facets() > 0 )
                {
                    ++mNumSideSets ;
                }
            }
...
//------------------------------------------------------------------------------
        ...
        ExodusWriter::populate_blocks( Progressbar * aProgressbar )
        {
            ...
            StringList tLabels( mBlocks.size() );
...
//------------------------------------------------------------------------------
        ...
        ExodusWriter::populate_sidesets( Progressbar * aProgessbar )
        {
            ...
            {
                inline bool
                operator()( const SideSet * aA, const SideSet * aB )
                {
                    return aA->id() < aB->id();
                }
            } opSideSetId ;
            ...
            Cell< SideSet * > tSideSets( tCount, nullptr );
            tCount = 0 ;
            for ( SideSet * tSideSet : mMesh->sidesets() )
            {
                if ( ! tSideSet->is_hidden() && tSideSet->number_of_facets() > 0 )
            ...
            for( SideSet * tSideSet : tSideSets )
            {
                ...
                for ( int64_t i = 0; i < tNumFacets; ++i )
                {
                    tSideSetIDs[ i ] = tSideSet->facet_by_index( i )->index_on_master() + 1;
                }
            ...
            for( uint k=0; k<tNumFields; ++k )
            {
                ...
                if( tField->write_field_to_file() )
                {
                    switch ( tField->entity_type())
                    {
                        ...
                        {
                            BELFEM_ASSERT( tField->data().length() == mMesh->number_of_nodes(),
                                "wrong field length for node field %s : is %lu expect %lu",
                                tField->label().c_str(), ( long unsigned int ) tField->data().length(), ( long unsigned int ) mMesh->number_of_nodes() );
                            ++tNodeFieldCount;
                            break;
                        }
                        case  EntityType::ELEMENT :
                        {
                            BELFEM_ASSERT( tField->data().length() == mMesh->number_of_elements(),
                                "wrong field length for element field %s : is %lu expect %lu",
                                tField->label().c_str(), ( long unsigned int ) tField->data().length(), ( long unsigned int ) mMesh->number_of_elements() );
...
//------------------------------------------------------------------------------
        ...
        ExodusWriter::populate_element_fields( Cell< mesh::Field * > & aFields )
        {
            ...
            uint tSize = tNumElementFields * tNumBlocks;
