...
namespace belfem
{
    namespace gasmodels
    {
         ...
         *
         * A thermodynamic state is fixed by ( T, p ), and the properties that
         * follow from it are expensive - a real gas equation of state solves
         * for v by Newton iteration before anything else can be evaluated. This
         * class holds one value slot per property together with a bit that says
         * whether the slot is valid. Changing the state clears every bit at
         * once, so each property is computed at most once per state and callers
         * may ask for them in any order.
         *
         * Two consequences are easy to get wrong. A slot that was never written
         * reads as its default rather than as an error, so a property must
         * always be requested through its accessor and never read from the
         * array directly. And anything that invalidates the state without
         * clearing the bits - changing the mixture composition, for instance -
         * leaves the cache holding values that belong to a gas that no longer
         * exists; remix() therefore has to reset it.
         */
        class Statevals
        {
...
//----------------------------------------------------------------------------
             ...
             */
            inline void
            update_Tp( const real T, const real p );
...
//----------------------------------------------------------------------------
             ...
             */
            inline void
            update_Tv( const real T, const real v );
...
//----------------------------------------------------------------------------
             ...
             */
            inline void
            update_pv( const real p, const real v );
...
//------------------------------------------------------------------------------
        void
        Statevals::update_Tp( const real T, const real p )
        {
            BELFEM_ASSERT( T > 0, "Invalid Temprerature" );
            BELFEM_ASSERT( p > 0, "Invalid Pressure" );
            if(       T != mStatevals[ BELFEM_STATEVAL_T ]
                   || p != mStatevals[ BELFEM_STATEVAL_P ] )
            {
                mStatebits.reset();

                mStatevals[ BELFEM_STATEVAL_T ] = T;
                mStatevals[ BELFEM_STATEVAL_P ] = p;
...
//----------------------------------------------------------------------------
        ...
        void
        Statevals::update_Tv( const real T, const real v )
        {
            if(       T != mStatevals[ BELFEM_STATEVAL_T ]
                   || v != mStatevals[ BELFEM_STATEVAL_V ] )
            {
                mStatebits.reset();

                mStatevals[ BELFEM_STATEVAL_T ] = T;
                mStatevals[ BELFEM_STATEVAL_V ] = v;
...
//----------------------------------------------------------------------------
        ...
        void
        Statevals::update_pv( const real p, const real v )
        {
            if(       p != mStatevals[ BELFEM_STATEVAL_P ]
                   || v != mStatevals[ BELFEM_STATEVAL_V ] )
            {
                mStatebits.reset();
