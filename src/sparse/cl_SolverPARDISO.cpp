...
//------------------------------------------------------------------------------
                ...
                Vector <real> & aRHS )
        {
            ...
            // backup indexing base
            SpMatrixIndexingBase tBase
                = static_cast< SpMatrixIndexingBase> ( aMatrix.indexing_base() );

            // test if we have the correct base
            bool tFlippedBase = false ;
            if ( tBase != mIndexingBase )
            {
            ...
            int_t tStatus = pardisotools_symbolic_factorization(
                aMatrix.n_rows(),
            ...
            // restore base if we changed it
            if ( tFlippedBase )
            {
                aMatrix.set_indexing_base( tBase );
            }
#else
...
//------------------------------------------------------------------------------
        ...
        int_t tStatus = pardisotools_solve(
                    aMatrix.n_rows(),
                    aMatrix.number_of_nonzeros(),
                    aRHS.n_cols(),
                    aMatrix.pointers(),
...
//------------------------------------------------------------------------------
        ...
        PARDISO::free()
        {
            ...
            {
                // tidy up memory
                int_t tStatus = pardisotools_free();
                ...
                if( tStatus != 0 )
                {
                    ...
                    BELFEM_ERROR( tStatus == 0,
                                 "PARDISO has thrown the error: %ld during memory freeing:\n%s",
                                 ( long ) tStatus,
                                 tMessage.c_str() );
                }
            }
...
//------------------------------------------------------------------------------
                             ...
                             const int_t aNumRhsColumns )
        {
            ...
            // backup indexing base ( restored at the end, see solve() —
            // leaking the Fortran base breaks the operator() contract of
            // every assembly that follows )
            SpMatrixIndexingBase tBase
                = static_cast< SpMatrixIndexingBase > ( aMatrix.indexing_base() );
            ...
            mParameters( 1 ) = ( int_t ) aMatrix.indexing_base() ;

            // matrix type is always real unsymmetric ( 11 ); the symmetric modes did not work so far
            mParameters( 2 ) = 11;
            ...
            int_t tStatus = pardisotools_initialize_parameters( mParameters.data() );
            ...
            if( tStatus != 0 )
            {
                ...
                // throw error
                BELFEM_ERROR( tStatus == 0,
                             "PARDISO has thrown the error: %ld during parameter initialization:\n%s",
                             ( long ) tStatus,
                             tMessage.c_str() );
            }
            ...
            if( tStatus != 0 )
            {
                ...
                // throw error
                BELFEM_ERROR( tStatus == 0,
                             "PARDISO has thrown the error: %ld during symbolic factorization:\n%s",
                             ( long ) tStatus,
                             tMessage.c_str() );
            }

            // restore base if we changed it ( same contract as solve() )
            if ( tBase != mIndexingBase )
            {
                aMatrix.set_indexing_base( tBase );
            }
...
//-----------------------------------------------------------------------------
        ...
        void
        PARDISO::check_status( const int_t aStatus )
        {
            if( aStatus != 0 )
            {
                // create the error message
                ...
                BELFEM_ERROR( aStatus == 0,
                             "PARDISO has thrown the error: %ld %s %ld:\n%s",
                             ( long ) aStatus,
                             tPhaseString.c_str(),
                             ( long ) mInfo( 0 ),
                             tMessage.c_str() );
...
//-----------------------------------------------------------------------------
        ...
        string
        PARDISO::error_message( const int_t aStatus ) const
        {
            switch( aStatus )
            {
                case(  0 ) :
