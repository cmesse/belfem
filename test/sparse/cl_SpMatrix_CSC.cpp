...
{
    // create the matrix
    SpMatrix tMatrix( gGraph, SpMatrixType::CSC );

    // Vector with pointers
    Vector<int> tPointers( tMatrix.n_cols() + 1 );
    ...
    std::memcpy( tPointers.data(), tMatrix.pointers(), ( tMatrix.n_cols() + 1 ) * sizeof( int ));
    std::memcpy( tIndices.data(), tMatrix.indices(), ( tMatrix.number_of_nonzeros()) * sizeof( int ));
    EXPECT_TRUE( tPointers == Vector<int>( { 1, 4, 7, 9, 12, 14 } ));
    EXPECT_TRUE( tIndices == Vector<int>(  { 1, 2, 4, 1, 2, 5, 3, 4, 1, 3, 4, 3, 5 } ));

    tMatrix.set_indexing_base( SpMatrixIndexingBase::Cpp );
    std::memcpy( tPointers.data(), tMatrix.pointers(), ( tMatrix.n_cols() + 1 ) * sizeof( int ));
    std::memcpy( tIndices.data(), tMatrix.indices(), ( tMatrix.number_of_nonzeros()) * sizeof( int ));
    EXPECT_TRUE( tPointers == Vector<int>( { 0, 3, 6, 8, 11, 13 } ));
    EXPECT_TRUE( tIndices == Vector<int>( { 0, 1, 3, 0, 1, 4, 2, 3, 0, 2, 3, 2, 4 } ));
    ...
    for ( uint k = 0; k < 2; ++k ) // make two runs, one in zero and one in one-based indexing
    {
        ...
        EXPECT_TRUE( tValues == Vector<real>( { 1., -2., -4., -1., 5., 8., 4., 2., -3., 6., 7., 4., -5. } ));
