...
// DISABLED: returns NaN across the whole temperature sweep, which starts at
// 100 K, below the lowest tabulated interval of every species here. The
// reference values were also taken against a locally modified thermo.inp, as
// the O3 line below still records, and that table is no longer shipped. Needs
// the NaN traced and the reference data re-derived against the current table
// before it means anything.
TEST( GASMODELS, DISABLED_Gibbs )
{
//------------------------------------------------------------------------------
/**
...
//------------------------------------------------------------------------------
    ...
    for( uint j=0; j<tM; ++j )
    {
        // bind the columns to vectors, otherwise the r2 overload
        // is ambiguous between the vector and the matrix version
        const Vector< real > tA( tValues.col( j ) );
        const Vector< real > tB( tExpect.col( j ) );

        EXPECT_NEAR( r2( tA, tB ),  1.0,  1e-2 );
    }
