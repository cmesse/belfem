...
// fractional range boundaries from trans.inp ( N2 low-temperature polys
// start at 63.7 K ) — regression gate for the Tmin/Tmax window clipping,
// audit finding C12
TEST( GASTABLES, input_transport_fractional_range )
{
    string tDataPath = gastables::data_path();

    gastables::InputTransport tTransport( tDataPath + "/trans.inp" );

    gastables::RefGas * tGas = new gastables::RefGas( "N2" );

    tTransport.read_data( tGas );

    real tEpsilon = 1e-9;

    EXPECT_NEAR( tGas->mViscosityPolys(0)->mTmin, 63.7, tEpsilon );
    EXPECT_NEAR( tGas->mConductivityPolys(0)->mTmin, 63.7, tEpsilon );
    EXPECT_NEAR( tGas->mConductivityPolys(0)->mTmax, 126.1, tEpsilon );
