...
### [closed/code_findings_from_comment_sweeps.md](closed/code_findings_from_comment_sweeps.md) — defects the comment auditors found while reading for contracts, parked for a code session

**CLOSED 2026-09-18** after six tiers (`70364c1`, `ca5fb51`, `93a2376`, `04a3e00`, `62746d9`, tier 6 the calculator's vector list), each through the plan + audit → code + audit loop with `make check` on both trees and Valgrind gates: the sideset broadcast, the undefined `IWG::N`, the null wrapper accessors, the `collect_node_coords` column bound, the include guard, seven ownership leaks on the setup and teardown paths, the basis dof container, the 3D boundary edges, the sideset tables built twice, the edge functions precomputed before the table rebuild, the block enrichment tables, the calculator's orphaned workspace vectors; three regression tests shipped. F8 refuted by two deep scans. Residue carried in the closed file: F21 (`Basis::set_sources` count-keyed `malloc( 0 )`, plan owed) and F7 (closed core, waits for Gregory).
...
**CLOSED 2026-09-16 (commit `08c306b`; O1 resolved: the T-matrix experiments dropped).** 56 scripts surveyed; the jury cut the keep set to 14 and withdrew every "exact generator" label but the TET10 zero-tests and `penta18.m`; landed under `src/fem/interpolation/doc/matlab/` with asserting drivers, a sympy comparison against the C++ tables, READMEs, a Doxygen exclusion and the repaired citations; verified under Octave. Originally: 23 proposed to keep (the TET10 and TRI6 Nédélec table generators and their zero-tests, the PENTA18 second-derivative verifier, the PYRA14 basis, the facet orientation studies behind the magic tables, the TET permutation generator), 34 dropped (duplicates, plots, scratch, abandoned alternatives, mutually inconsistent face tables). Two dangling citations in `nedelec_derivation.md` get repaired. Cleanup to the commenting guideline, a README per directory, a Codex sweep, then the verifiers run headless in MATLAB as the gate. The T-matrix experiments are an open question for Christian.

### [closed/comment_cleanup_sweep_1.md](closed/comment_cleanup_sweep_1.md) — bring the existing `src/` comments under `doc/commenting_guidelines.md`

**CLOSED 2026-09-16 (commits `caf2045`, `05a4eb5`, `08c306b`; `make check` both backends, comment-only gate over the range; sweep 2 follows from its §9).** Originally planned as comment-only commits, one per module: first the four comments that are wrong (a "FACE IDs" heading above a cell-dof loop, a "receive complete" above a send's `MPI_Wait`, a derivative contract one branch violates, a "lower triangle" MUMPS note), then the ~130 dated and ~30 reviewer-credited comments rewritten by hand (the date names the devlog that holds the experiment), then ~110 commented-out statements, 31 TODOs triaged (10 more are in the closed cohomology core and are listed for Gregory), then narration by module (`cl_FEM_DofMgr_DofData.cpp` 251 candidates, `commtools.hpp` 221), then the `@brief`-restates-the-name blocks in the materials and IWG headers. Gates: a comment-only checker (`g++ -fpreprocessed` token diff; the "identical binary" gate was refuted, `BELFEM_ERROR` embeds `__LINE__`), `make check`, a per-batch Codex language sweep, a permanent census script. Banners out of scope by ruling. Sweep 2 (missing contracts, Grok + Codex second-tier list) is written only after this closes.
