...
**Status:** IN PROGRESS — approved by Christian 2026-09-16; R0 and B1 done the same day (checker positive/negative controls passed in C++ and Fortran; B1 comment-only gate OK); uncommitted; Codex sweep of B1 applied (one real catch, see D2); B2a done (47 → 0 dated/credited lines in the controller; both halves gated; both halves Codex-swept and applied); B2b done and Codex-swept (three sweep suggestions declined with reasons, one factual catch applied); B2c done and Codex-swept; B2d done (24 sites) and Codex-swept; B2e done (9 sites) and Codex-swept; B2f done (24 sites in 20 files) and Codex-swept — **B2 complete: zero dated / credited / record-pointing comment lines in `src/` outside strings, the closed core and the CLion bylines (O6)**; B3 done (about 270 lines, 46 files, age-checked to the re-init root) and Codex-checked for deleted reasons (pass); B4 done and Codex-swept; B5a and B5b done and Codex-checked; B5c done and Codex-checked (D5 found and fixed); B5d done and Codex-checked; B5e done and Codex-checked; B5f and B5g done and Codex-checked (three restored reasons, seven survivors removed); **B5 complete**: comment lines 31,723 → 27,839, narration candidates 3,023 → 1,288 (what remains is below the 20-per-file threshold or kept as stage headings); B6 done and Codex-checked (one real catch: the collectiveness statements on `comm_barrier`/`broadcast` went with the deleted briefs, restored as one-line contracts; `collect` overloads now say they only receive; two survivors rewritten as contracts); B6c `make doc` spot-check is Christian's; **R9 closing gate next**, then a new B2f for the ~30 sites in modules B2 did not enumerate.
